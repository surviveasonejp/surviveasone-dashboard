...
const PHASE_STATUS: Array<{ phase: string; label: string; status: PhaseStatus; items: string[]; remaining?: string[] }> = [
  ...
  },
  {
    ...
    status: "completed" as const,
    items: ["要配慮者チェックリスト(6カテゴリ)", "行動チェックリスト(5カテゴリ)", "住居形態別ガイド", "FAQ構造化データ(10問)", "パーソナライズフィルタ", "Xシェア機能", "アクセシビリティ(ARIA)", "オフライン強化(SW v3)"],
  },
  {
    phase: "Phase 10",
  ...
  },
  {
    ...
    items: [
      ...
      "realEvents に affectedPopulation フィールド追加（紙おむつ850万人等の影響規模を一次統計出典付きで記録）",
    ],
  },
  {
    phase: "Phase 23",
    label: "公開VTS/港湾EDIでタンカー調査精度向上",
    status: "completed" as const,
    items: [
      "国交省 海保VTS 3ルート（浦賀水道/明石海峡/関門海峡）と名古屋港EDIから入航予定タンカーを自動取得",
      "mlit-vts-fetcher.ts 汎用化（ルート別列マッピング・油タンカー/ガスタンカー/ケミカルタンカー抽出）",
      "nagoya-port-fetcher.ts（プロダクトオイル/LNG/ケミカル/油送船抽出・コールサイン取得）",
      "/api/port-arrivals?port=uraga|akashi|kanmon|nagoya 追加・tankers.json 未登録便を正規化比較で自動検出",
      "Cron: UTC 6:00/18:00×2/日でVTS 3ルート、18:00で名古屋港も相乗り",
      "ops 連携: /api/port-arrivals 日次ポーリング・新規便をDiscord通知（KV 14日重複排除）",
      "AIS 途絶・偽装・沈黙船でも VTS 管制情報で補完可能に",
    ],
  },
  {
    phase: "Phase 24",
    label: "X投稿戦略見直し＋LLM補助",
    status: "completed" as const,
    items: [
      "4つの新テンプレート追加（household/population_impact/market_status/port_arrivals_daily）",
      "/api/real-events と /api/port-arrivals から本番実データを自動注入するビルダー関数",
      "曜日ベース発火（日曜=household、火/金=port_arrivals_daily、毎日=population_impact 重複排除付き）",
...
const SIMULATION_FEATURES = [
  ...
  { label: "封鎖解除曲線", desc: "国際協調: 7日→30日で解除(残留10%) / 標準対応: 30日→120日(残留30%) / 需要超過: 90日→365日(残留60%) / 停戦・回復: 45日目停戦宣言→保険解除→港湾再開→契約再締結→残留8%(5段階セグメント補間)" },
        ...
        <div className="bg-panel border border-border rounded-lg p-6 space-y-4">
          ...
          {!canInstall && platform === "ios" && (
            <p className="text-neutral-400 text-sm">
              Safari の <strong>共有ボタン（↑）</strong> →「ホーム画面に追加」を選択してください。
            </p>
          )}
          {!canInstall && platform === "android-chrome" && (
            <p className="text-neutral-400 text-sm">
              Chrome の <strong>メニュー（⋮）</strong> →「ホーム画面に追加」を選択してください。
            </p>
          )}
        </div>
      )}
